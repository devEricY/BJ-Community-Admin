                                                    ...
                                                    <c:forEach var="bdList" items="${bdList}" varStatus="status">
                                                        <c:if test="${status.count > 0}" >
                                                            <tr>
                                                                ...
                                                                <td><a class="infoLike" >${bdList.title}</a></td>
                                                                <td><a class="infoLike" >${bdList.admin_id}</a></td>
                                                                <td><a class="infoLike" >${bdList.admin_nick}</a></td>
                                                                <td><a class="infoLike" >${bdList.view_cnt}</a></td>
                                    ...
                                    <ul style="list-style: none;height: 30px;display: inline-block;padding-left: 0px;padding-right: 0px;margin-right: 0px;margin-left: 0px;">
                                        ...
                                        <c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage}" var="pageNum">
                                            <c:if test="${curPage == pageNum}" >
                                                <li class="on">
                                                    <a href='<c:url value="/notice/list?page=${pageNum}"/>'>${pageNum}</a>
                                                </li>
                                            </c:if>
                                            <c:if test="${curPage != pageNum}" >
                                                <li>
